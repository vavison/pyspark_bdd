...
from tests.steps.step_utils import assert_spark_dataframe_equal, gherkin_table_str_to_df
...
def parse_transactions(spark: SparkSession, table: str) -> DataFrame:
    return gherkin_table_str_to_df(spark, table)
...
def verify_result(spark: SparkSession, table: str, result: DataFrame):
    expected_result = gherkin_table_str_to_df(spark, table)
